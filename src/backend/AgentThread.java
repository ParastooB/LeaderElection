...
//			while (getParent().isVisible() && !parent.isElectionComplete()) {
				...
				if (parent.getBelievers() == parent.AGENT_COUNT - 1) {
					System.out.println("Thread " + iAgent.getAID() + " finished at round " + parent.roundsCount()
							+ ", threads remaining: " + Thread.activeCount());
					...
					// LOCK
					parent.rounds();
					if (iAgent.getLeaderAID() == parent.leader().getAID()
							&& iAgent.getAID() != parent.leader().getAID())
